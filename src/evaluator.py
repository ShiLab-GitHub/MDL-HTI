...
def writetofile_fn(args, res_2hop, res_random=None, data_type="test"):
    ...
    columns = ["algo", "data", 'h', "eh", "ah", "lr", "wd", "nl", "nh", "ne", "p", "drp", "c_drp", "tr", "#ctx", "pl", "ps"
    , "bs", "l", "o", "si", "s", "c", "k", "cls", "infm"]
    ...
    save_folder = "results/" + '_'.join([str(now.month).zfill(2), str(now.day).zfill(2)]) + str(now.strftime("%H.%M.%S")) + '/'
    
    # 检查并创建目录
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    nsave_str = save_folder + args.algorithm + "_" + args.dataset + "_" + data_type
