...
def calculate_scores(edge_list, confidence, labels):
    ...
    # 计算标签中1的比例，并根据标签比例来设定阈值
    positive_ratio = np.sum(labels) / float(len(labels))
    threshold_value = np.quantile(confidence, 1 - positive_ratio)
    # 使用计算出的阈值进行二分类预测
    pred = np.where(confidence > threshold_value, 1, 0)

    # 计算F1分数
    precision, recall, f1, _ = metrics.precision_recall_fscore_support(labels, pred, average='macro')
...
def writetofile_fn(args, res_2hop, res_random=None, data_type="test"):
    ...
    result_keys = ["roc_auc", "auc_pr", "mr", "mrr", "hits@1", "hits@3", "hits@10", 'recall', 'precision',  "acc", "f1"]
...
class Trainer():
    ...
    def choose_best(self, dict, data_type=''):
        result_keys = ["roc_auc", "auc_pr", "mr", "mrr", "hits@1", "hits@3", "hits@10", 'recall', 'precision',  "acc", "f1"]
        if data_type == 'train':
            self.res_train["roc_auc"] = dict["roc_auc"] if dict["roc_auc"] > self.res_train["roc_auc"] else \
            self.res_train["roc_auc"]
            self.res_train["auc_pr"] = dict["auc_pr"] if dict["auc_pr"] > self.res_train["auc_pr"] else self.res_train[
                "auc_pr"]
            ...
            self.res_train["hits@1"] = dict["hits@1"] if dict["hits@1"] > self.res_train["hits@1"] else self.res_train[
                "hits@1"]
            self.res_train["hits@3"] = dict["hits@3"] if dict["hits@3"] > self.res_train["hits@3"] else self.res_train[
                "hits@3"]
            self.res_train["hits@10"] = dict["hits@10"] if dict["hits@10"] > self.res_train["hits@10"] else \
            self.res_train["hits@10"]
            self.res_train["recall"] = dict["recall"] if dict["recall"] > self.res_train["recall"] else \
                self.res_train["recall"]
            self.res_train["precision"] = dict["precision"] if dict["precision"] > self.res_train["precision"] else \
                self.res_train["precision"]
            self.res_train["acc"] = dict["acc"] if dict["acc"] > self.res_train["acc"] else \
                self.res_train["acc"]
            self.res_train["f1"] = dict["f1"] if dict["f1"] > self.res_train["f1"] else \
                self.res_train["f1"]
        elif data_type == 'valid':
            self.res_valid["roc_auc"] = dict["roc_auc"] if dict["roc_auc"] > self.res_valid["roc_auc"] else \
            self.res_valid["roc_auc"]
            self.res_valid["auc_pr"] = dict["auc_pr"] if dict["auc_pr"] > self.res_valid["auc_pr"] else self.res_valid[
                "auc_pr"]
            ...
            self.res_valid["hits@1"] = dict["hits@1"] if dict["hits@1"] > self.res_valid["hits@1"] else self.res_valid[
                "hits@1"]
            self.res_valid["hits@3"] = dict["hits@3"] if dict["hits@3"] > self.res_valid["hits@3"] else self.res_valid[
                "hits@3"]
            self.res_valid["hits@10"] = dict["hits@10"] if dict["hits@10"] > self.res_valid["hits@10"] else \
                self.res_valid["hits@10"]
            self.res_valid["recall"] = dict["recall"] if dict["recall"] > self.res_valid["recall"] else \
                self.res_valid["recall"]
            self.res_valid["precision"] = dict["precision"] if dict["precision"] > self.res_valid["precision"] else \
                self.res_valid["precision"]
            self.res_valid["acc"] = dict["acc"] if dict["acc"] > self.res_valid["acc"] else \
                self.res_valid["acc"]
            self.res_valid["f1"] = dict["f1"] if dict["f1"] > self.res_valid["f1"] else \
                self.res_valid["f1"]
        elif data_type == 'test_2hop':
            self.res_2hop["roc_auc"] = dict["roc_auc"] if dict["roc_auc"] > self.res_2hop["roc_auc"] else self.res_2hop[
                "roc_auc"]
            self.res_2hop["auc_pr"] = dict["auc_pr"] if dict["auc_pr"] > self.res_2hop["auc_pr"] else self.res_2hop[
                "auc_pr"]
            ...
            self.res_2hop["hits@1"] = dict["hits@1"] if dict["hits@1"] > self.res_2hop["hits@1"] else self.res_2hop[
                "hits@1"]
            self.res_2hop["hits@3"] = dict["hits@3"] if dict["hits@3"] > self.res_2hop["hits@3"] else self.res_2hop[
                "hits@3"]
            self.res_2hop["hits@10"] = dict["hits@10"] if dict["hits@10"] > self.res_2hop["hits@10"] else self.res_2hop[
                "hits@10"]
            self.res_2hop["recall"] = dict["recall"] if dict["recall"] > self.res_2hop["recall"] else \
                self.res_2hop["recall"]
            self.res_2hop["precision"] = dict["precision"] if dict["precision"] > self.res_2hop["precision"] else \
                self.res_2hop["precision"]
            self.res_2hop["acc"] = dict["acc"] if dict["acc"] > self.res_2hop["acc"] else \
                self.res_2hop["acc"]
            self.res_2hop["f1"] = dict["f1"] if dict["f1"] > self.res_2hop["f1"] else \
                self.res_2hop["f1"]
        elif data_type == 'test_random':
            self.res_random["roc_auc"] = dict["roc_auc"] if dict["roc_auc"] > self.res_random["roc_auc"] else \
            self.res_random["roc_auc"]
            self.res_random["auc_pr"] = dict["auc_pr"] if dict["auc_pr"] > self.res_random["auc_pr"] else \
            self.res_random["auc_pr"]
            ...
            self.res_random["hits@1"] = dict["hits@1"] if dict["hits@1"] > self.res_random["hits@1"] else \
            self.res_random["hits@1"]
            self.res_random["hits@3"] = dict["hits@3"] if dict["hits@3"] > self.res_random["hits@3"] else \
            self.res_random["hits@3"]
            self.res_random["hits@10"] = dict["hits@10"] if dict["hits@10"] > self.res_random["hits@10"] else \
            self.res_random["hits@10"]
            self.res_random["recall"] = dict["recall"] if dict["recall"] > self.res_random["recall"] else \
                self.res_random["recall"]
            self.res_random["precision"] = dict["precision"] if dict["precision"] > self.res_random["precision"] else \
                self.res_random["precision"]
            self.res_random["acc"] = dict["acc"] if dict["acc"] > self.res_random["acc"] else \
                self.res_random["acc"]
            self.res_random["f1"] = dict["f1"] if dict["f1"] > self.res_random["f1"] else \
                self.res_random["f1"]

    def __init__(self, params, dl, graph_classifier, valid_data, train_data, valid_evaluator=None,
    ...
    def reset_training_state(self):
        self.early_stopping = EarlyStopping(patience=self.params.patience, verbose=True,
                                            save_path='checkpoints/pgrail_' + self.params.save_str + '.pt')
        self.res_train, self.res_valid, self.res_2hop, self.res_random = defaultdict(float), defaultdict(
            float), defaultdict(float), defaultdict(float)
        self.res_train["roc_auc"], self.res_valid["roc_auc"], self.res_2hop["roc_auc"], self.res_random[
            "roc_auc"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["auc_pr"], self.res_valid["auc_pr"], self.res_2hop["auc_pr"], self.res_random[
            "auc_pr"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["mr"], self.res_valid["mr"], self.res_2hop["mr"], self.res_random["mr"] = 1e5, 1e5, 1e5, 1e5
        self.res_train["mrr"], self.res_valid["mrr"], self.res_2hop["mrr"], self.res_random[
            "mrr"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["hits@1"], self.res_valid["hits@1"], self.res_2hop["hits@1"], self.res_random[
            "hits@1"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["hits@3"], self.res_valid["hits@3"], self.res_2hop["hits@3"], self.res_random[
            "hits@3"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["hits@10"], self.res_valid["hits@10"], self.res_2hop["hits@10"], self.res_random[
            "hits@10"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["recall"], self.res_valid["recall"], self.res_2hop["recall"], self.res_random[
            "recall"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["acc"], self.res_valid["acc"], self.res_2hop["acc"], self.res_random[
            "acc"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["f1"], self.res_valid["f1"], self.res_2hop["f1"], self.res_random[
            "f1"] = -1e5, -1e5, -1e5, -1e5
        self.res_train["precision"], self.res_valid["precision"], self.res_2hop["precision"], self.res_random[
            "precision"] = -1e5, -1e5, -1e5, -1e5
    ...
    def train_epoch(self, epoch):
        ...
        for b_idx, batch in enumerate(tqdm(dataloader)):
            ...
            score_pos, score_neg, cluster_loss, s_logits, attn, mp, view_embeds, view_aggr, rep_rel = self.graph_classifier(
                data_pos, data_neg, self.dl, "t")
            ...
            rk_loss = self.ranking(score_pos, score_neg.view(len(score_pos), -1).mean(dim=1),
                                   torch.Tensor([self.params.margin]).to(device=self.params.device))
            loss = rk_loss
            if int(self.params.clustering) in [-3, 2, 3, 1, 4]:
                ...
                if int(self.params.infomax) == 1:
                    ...
                    loss += self.params.summary_coeff * s_loss
            num_pos_samples = score_pos.size(0)
            num_neg_samples = score_neg.size(0)
            pos_labels = torch.ones(num_pos_samples, 1)  # 正样本标签为1, 注意给予正确的维度
            neg_labels = torch.zeros(num_neg_samples, 1)  # 负样本标签为0, 维度为 [n, 1]

            # 将标签和得分合并
            label = torch.cat((pos_labels, neg_labels), dim=0)
            score = torch.cat((score_pos, score_neg), dim=0)


            # 确保标签和分数的维度一致
            assert label.size() == score.size(), "标签和分数的尺寸必须匹配"

            # 定义二元交叉熵损失函数
            criterion = nn.BCEWithLogitsLoss()

            # 计算损失
            bce_loss = criterion(score, label)

            loss = bce_loss + rk_loss
            # 记录损失值
            # print(f"Epoch {epoch},rk_loss: {rk_loss.item()}, bce_loss: {bce_loss.item()}, total_loss: {loss.item()}")
            ...
            # print(f"Value [first 50 elements]:\n{param.data.flatten()[:50]}")  # 打印参数的前5个值作为示例
            # if param.grad is not None:
            # print(f"Gradient [first 50 elements]:\n{param.grad.flatten()[:50]}")  # 打印梯度的前5个值
            # else:
            ...
            if self.valid_evaluator and self.params.eval_every_iter and self.updates_counter % self.params.eval_every_iter == 0 and (
                    (b_idx > 0 and epoch == 1) or (epoch > 1)):
                with torch.no_grad():
                    tic = time.time()
                    result_dict, pos_scores, neg_scores, v_cluster_loss, vs_logits = self.valid_evaluator.eval(
                        data_type="valid", save=False)
                    self.choose_best(result_dict, data_type='valid')

                    v_rk_loss = self.ranking(torch.Tensor(pos_scores).to(device=self.params.device),
                                             torch.Tensor(neg_scores).view(len(pos_scores), -1).mean(dim=1).to(
                ...
                logging.info('\nValidation Performance: ' + ' val_loss = ' + str(val_loss) + ' roc_auc = ' + str(
                    result_dict['roc_auc']) + ' auc_pr = ' + str(result_dict['auc_pr']) + ' acc = ' + str(
                    result_dict['acc']) + ' f1 = ' + str(result_dict['f1']) + ' mrr = ' + str(
                    result_dict['mrr']) + ' hits@1 = ' + str(result_dict['hits@1']) + ' in ' + str(time.time() - tic))
    ...
    def train(self):
        ...
        for epoch in range(1, self.params.num_epochs + 1):
            ...
            print(f'Epoch {epoch} with train loss: {loss} in {time_elapsed}')
            if self.early_stopping.early_stop:
                print('Early stopping!')
                break
...
def test(params, dl):
    ...
    if len(dl.t_2_pos) > 0:  # params.dataset not in ['dblp', 'imdb']:
        ...
        for r in range(1, params.runs + 1):
            ...
            if not os.path.exists(os.path.join(params.db_path, 'data' + '.mdb')):
                generate_subgraph_datasets(params, dl, splits=['test_2hop'], saved_relation2id=None,
                                           max_label_value=graph_classifier.gnn.max_label_value)
            test_2hop = SubgraphDataset(params, params.db_path, dl, 'test_pos', 'test_neg',
        ...
        logging.info('\nAvg test Set Performance -- mean auc_pr :' + str(np.mean(all_auc_pr)) + ' std auc_pr: ' + str(
            np.std(all_auc_pr)))
    ...
    if len(dl.t_r_pos) > 0:
        ...
        for r in range(1, params.runs + 1):
            ...
            if not os.path.exists(os.path.join(params.db_path, 'data' + '.mdb')):
                generate_subgraph_datasets(params, dl, splits=['test_random'], saved_relation2id=None,
                                           max_label_value=graph_classifier.gnn.max_label_value)
            test_random = SubgraphDataset(params, params.db_path, dl, 'test_pos', 'test_neg',
                                          add_traspose_rels=params.add_traspose_rels,
                                          num_neg_samples_per_link=params.num_neg_samples_per_link)
    ...
    if len(list(dl.ntest_neg.values())[0][0]) > 0:
        ...
        for r in range(1, params.runs + 1):
            ...
            if not os.path.exists(os.path.join(params.db_path, 'data' + '.mdb')):
                generate_subgraph_datasets(params, dl, splits=['test'], saved_relation2id=None,
                                           max_label_value=graph_classifier.gnn.max_label_value)
            test_r = SubgraphDataset(params, params.db_path, dl, 'test_pos', 'test_neg',
                                     add_traspose_rels=params.add_traspose_rels,
                                     num_neg_samples_per_link=params.num_neg_samples_per_link)
