...
class hetero_data:
    ...
    def build_graph(self, raw_examples, train=False):
        ...
        if train:
            for _raw_ex in raw_examples:#训练考虑多种连接关系，但是与herb数据无关
        ...
        return subj2objs, obj2subjs#起始点+边的关系，查目标点
    ...
    def calculate_hin_stats(self):
        ...
        self.adj = torch.sparse_coo_tensor(i, v, torch.Size(shape)).requires_grad_(False).to(self.args.device)
        ...
        v = torch.FloatTensor(d)
        shape = x.shape
        self.I = torch.sparse_coo_tensor(i, v, torch.Size(shape)).requires_grad_(False).to(self.args.device)
    ...
    def load_links_tv(self, args, path_name, file_name_link='link.dat', file_name_meta='meta.dat', type=None):
        ...
        df = edges_df.groupby('r_id', sort=True).apply(lambda x: x.reset_index(drop=True)).drop('r_id', axis=1).reset_index()#根据边的关系分组后移除边
        ...
        for index, row in df.iterrows():#df.iterrows()迭代过程中可以包含原来有的
            ...
            t_idx = entity_2id[t_id]
            r_idx = rel_2id[r_id]
            edges_train['mat'][r_idx].append((h_idx, t_idx, link_weight))#三元组保存
            edges_train['edges'][r_idx][tuple([h_idx, t_idx])] = link_weight#字典
            edges_train['count'][r_idx] += 1
            edges_train['total'] += 1
        self.entity_2id["train"]["entity_2id"] = entity_2id#类的参数
        ...
        # 生成反向映射 index_2entity
        index_2entity = {index: entity for entity, index in entity_2id.items()}

        # 指定要保存文件的路径和文件名
        save_path = "../data/ddb/herb_data/"  # 请替换为实际的路径
        file_name = "idx_to_entity.json"  # 文件名称

        # 组合完整的文件路径
        full_file_path = os.path.join(save_path, file_name)

        # 将index_2entity字典保存到文件中
        with open(full_file_path, "w") as file:
            json.dump(index_2entity, file)

        print(f"文件已被保存在 {full_file_path}")
