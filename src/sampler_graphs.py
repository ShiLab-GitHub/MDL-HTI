...
def links2subgraphs(A, graphs, args, max_label_value=None):
    ...
    enc_ratios = []
    num_pruned_nodes = []
    BYTES_PER_DATUM = get_average_subgraph_size(100, list(graphs.values())[0]['pos'], A, args) * 1
    links_length = 0
    for split_name, split in graphs.items():
        links_length += (len(split['pos']) + len(split['neg'])) * 2
    map_size = links_length * BYTES_PER_DATUM*1.17715
    #
    
    
    #############################################################################################
    # If you change the training parameters, this section should be adjusted accordingly.
    env = lmdb.open(args.db_path, map_size=8122405755, max_dbs=6)# for Ablation
    env = lmdb.open(args.db_path, map_size=122405642, max_dbs=6)# for comparison
    ############################################################################################
