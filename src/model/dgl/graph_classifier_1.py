...
def return_name(x):
    df = pd.read_csv('../data/ddb/herb_data/entity2500.txt', sep="\t", header=None)
    return df.loc[df[df.columns[1]] == x, df.columns[0]].iloc[0]


herb_code_df = pd.read_csv('../data/ddb/herb_data/codes_one_2500.txt', sep="\t", header=None)
...
class GraphClassifier(nn.Module):
    ...
    def forward(self, data_pos, data_neg, dl, type="t"):
        ...
        g_rep = torch.cat([head_embs.view(-1,  self.params.num_layers * self.params.hidden_dim), tail_embs.view(-1,  self.params.num_layers * self.params.hidden_dim), rel_embs, view_aggr, code1], dim=1)
        ...
        g_rep1 = torch.cat([head_embs1.view(-1,  self.params.num_layers * self.params.hidden_dim), tail_embs1.view(-1,  self.params.num_layers * self.params.hidden_dim), rel_embs1, view_aggr1, code3], dim=1)
